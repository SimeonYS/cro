...
class CroPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('cro.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cro`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cro WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cro (date, title, link, content)"
